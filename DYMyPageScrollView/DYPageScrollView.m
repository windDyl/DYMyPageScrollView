...
@interface DYPageScrollView ()<UIScrollViewDelegate>
...
@implementation DYPageScrollView
...
- (void)autoNextPage {
    ...
    switch (self.orientation) {
        case DYPageScrollViewOrientationHorizontal:{
            
            [_scrollView setContentOffset:CGPointMake(self.page * _pageSize.width, 0) animated:YES];
            break;
        }
        case DYPageScrollViewOrientationVertical:{
    ...
    switch (self.orientation) {
        case DYPageScrollViewOrientationHorizontal:{
            ...
            for (int i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                DYPageCell *cell = [_cells objectAtIndex:i];
...
- (void)setPageAtIndex:(NSInteger)pageIndex{
    ...
    if ((NSObject *)cell == [NSNull null]) {
        cell = [_dataSource pageScrollView:self cellForPageAtIndex:pageIndex % self.orginPageCount];
...
- (void)reloadData
{
    ...
    if (_needsReload) {
        ...
        //重置pageCount
        if (_dataSource && [_dataSource respondsToSelector:@selector(numberOfPagesInPageScrollView:)]) {
            
            //原始页数
            self.orginPageCount = [_dataSource numberOfPagesInPageScrollView:self];
            
            //总页数
            _pageCount = self.orginPageCount == 1 ? 1: [_dataSource numberOfPagesInPageScrollView:self] * 3;
        ...
        //重置pageWidth
        if (_delegate && [_delegate respondsToSelector:@selector(sizeForPageInPageScrollView:)]) {
            _pageSize = [_delegate sizeForPageInPageScrollView:self];
        }
        ...
        switch (self.orientation) {
            case DYPageScrollViewOrientationHorizontal://横向
...
- (UIView *)dequeueReusableCell{
    DYPageCell *cell = [_reusableCells lastObject];
    if (cell) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if ([_delegate respondsToSelector:@selector(didScrollToPage:inPageScrollView:)] && _currentPageIndex != pageIndex) {
        [_delegate didScrollToPage:pageIndex inPageScrollView:self];
    }
